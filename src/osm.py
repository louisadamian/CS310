...
def get_points(filepath="umb_point_data.geojson"):
    """
    ...
    query = 'node(poly: "' + __coords_list_to_str(UMB_REGION) + '");'
...
def get_ways(filepath="umb_way_data.geojson")->dict:
    ...
    """
    query = 'way["highway"~"pedestrian|footway|steps|sidewalk|cycleway"](poly: "' + __coords_list_to_str(UMB_REGION) + '");(._;>;);out;'
    print(query)
    if os.path.isfile(filepath):
        with open(filepath, "r") as f:
            return json.loads(f.read())
